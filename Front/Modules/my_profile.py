...
def run(frame_parent):
    ...
    module_frame = criar_frame(frame_parent, 0, 0, "news", co0, co1, 0, 0, 0)
    ...
    frame_ratings_header = criar_frame(frame_section_ratings, 0, 0, "we", co3, co3, 0, 0, 0)
    frame_ratings_header.columnconfigure(0, minsize = 0, weight = 1)
    # frame_ratings_header.rowconfigure(0, minsize = 0, weight = 1)

    frame_ratings_title = criar_frame(frame_ratings_header, 0, 0, "we", co3, co3, 0)
    # frame_ratings_title.columnconfigure(0, minsize = 0, weight = 1)
    # frame_ratings_title.rowconfigure(0, minsize = 0, weight = 1)
    criar_label(frame_ratings_title, 'Avaliações', 'Calibri, 20', co3, 0, 0, 'nes').configure(fg='white')
    ...
    frame_listas = criar_frame(frame_section_ratings, 1, 0, "nsew", co0, co1, 1)
    ...
    for i, grade in enumerate(grades):
        ...
        frame_lista = criar_frame(frame_listas, i, 0, "news", co1, co1, 1, 0, 0)
        ...
        frame_parent_users = criar_frame(frame_lista, 1, 0, "news", co1, co1, 1, 0, 0)
        # frame_parent_users.configure(bg=lista_colors[i])
        ...
        for j, user in enumerate(grade):
            ...
            frame_user = criar_frame(frame_parent_users, j, 0, 'nsew', co1, co1, 0)
            # frame_user.configure(background='red')
            frame_user.columnconfigure(0, weight=1)

            frame_user_data = criar_frame(frame_user, 0, 0, 'ew', co1, co1, 0, 0, 0)
            frame_user_data.columnconfigure(0, weight=1)

            criar_label(frame_user_data, user['name'], 'Calibri, 12', co1, 0, 0, "w")  # linha para teste
            criar_label(frame_user_data, get_role_name(user['role_id']), 'Calibri, 10', co1, 1, 0, "w")  # linha para teste
...
def criar_label(quadro, text, font, background, r, c, sticky='n'):
    ...
    widget = Label(quadro, text=text, font=font, background = background , justify='left')
...
def criar_button(quadro, text, font, r, c, command, sticky='ne'):
    ...
    widget = Button(quadro, text = text, font = font, background = co0, justify='right', fg=co2, command=command,
        width=13, height=0, activebackground='#c5a8b0')
    widget.grid(row=r, column=c, sticky= sticky)
    return widget
...
def graphic_pie(data=list, labels = list):
    ...
    wedges, texts, autotexts = ax.pie(data, autopct=lambda pct: func(pct, data), textprops=dict(color="white"), colors=[col_to_rate, col_rated])
