...
def run(frame_parent):
    ...
    textojanela('lbl_titulo', 'Cadastro de Grupos',
                24, 0.02, 0.01, 0.96, 0.09, co0, co3)
    ...
    # Widgets de entrada
    ent_lider = Entry(master=frm_grupo, width=30, fg=co2,
                      font=('Calibre 13'))  # Nome Líder
    ent_lider.grid(row=0, column=1, padx=5)

    ent_lemail = Entry(master=frm_grupo, width=30, fg=co2,
                       font=('Calibre 13'))  # E-mail Líder
    ent_lemail.grid(row=0, column=3, padx=5)

    ent_client = Entry(master=frm_grupo, width=30, fg=co2,
                       font=('Calibre 13'))  # Nome Client
    ent_client.grid(row=1, column=1, padx=5)

    ent_cemail = Entry(master=frm_grupo, width=30, fg=co2,
                       font=('Calibre 13'))  # E-mail Client
    ...
    widgetlabel('lbl_lider', 0, 0, 'Nome do Líder do Grupo:')
    # Widget de texto e-mail Líder do Grupo
    widgetlabel('lbl_lemail', 0, 2, 'E-mail do Líder do Grupo:')
    # Widget de texto nome Fake Client
    widgetlabel('lbl_client', 1, 0, 'Nome do Fake Client:')
    # Widget de texto e-mail Fake Client
    widgetlabel('lbl_cemail', 1, 2, 'E-mail do Fake Client:')
    ...
    # Comando xview para orientação HORIZONTAL
    scroll_tree = ttk.Scrollbar(
        frm_tabela, orient=VERTICAL, command=tree.yview)
    scroll_tree.pack(side=RIGHT, fill=Y)
    # xscrollcomand para barra horizontal
    tree.configure(yscrollcommand=scroll_tree.set)
    # Seleciona qual parte do canvas o scrollbar deve identificar
    tree.bind('<Configure>', lambda e: tree.configure(
        scrollregion=tree.bbox('all')))
